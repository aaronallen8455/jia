...
-- Search function
DELIMITER $$
CREATE PROCEDURE search (search VARCHAR(39), showEvents TINYINT(1) UNSIGNED, showProfiles TINYINT(1) UNSIGNED, showVenues TINYINT(1) UNSIGNED, page_num SMALLINT UNSIGNED)
    BEGIN
        DECLARE num_results SMALLINT UNSIGNED DEFAULT 31;
        DECLARE num_offset SMALLINT UNSIGNED;
        SET num_offset = (num_results-1)*(page_num-1);
        ...
        UNION
        (SELECT 3.5, 'profile', users.id, CONCAT_WS(' ', users.first_name, users.last_name), profiles.bio, NULL, NULL, NULL, NULL
        ...
        ORDER BY score DESC, `date` DESC LIMIT num_offset, num_results;
    END $$
DELIMITER ;